console.info("[KasaFlow Payroll] PostgreSQL API build 2.3.14");
...
function clearMigrationToken() {
  try { localStorage.removeItem(MIGRATION_TOKEN_KEY); } catch {}
}
...
async function apiFetch(path, options = {}) {
  ...
  const response = await fetch(MIGRATION_API_BASE + requestPath, {
    method,
    headers: {
      ...headers,
  ...
  if (response.status === 401) {
    clearMigrationToken();
    try { window.parent.postMessage({ type: "garageflow:auth-required" }, location.origin); } catch (_) {}
  }
...
// PUT başarı döndükten sonra kısa süreli "commit koruması".
// Amaç: ödeme gerçekten kaydedilmişken hemen arkasından gelen eski/stale GET
// cevabının personeli tekrar "maaşı bekliyor" listesine sokmasını engellemek.
const recentSalaryCommits = new Map();
const SALARY_COMMIT_GUARD_MS = 10 * 60 * 1000;


// Kalıcı istemci ödeme işareti.
// Backend ödeme kaydını başarıyla kabul ettikten sonra aynı dönem, API GET cevabı
// gecikse veya farklı bir id biçimiyle gelse bile yeniden "bekliyor" görünmesin.
const SALARY_PAID_MARKERS_KEY = "kasaflow_salary_paid_markers_v1";
const SALARY_PAID_MARKER_MAX_AGE_MS = 120 * 24 * 60 * 60 * 1000;

function normalizePayrollName(value) {
  return String(value || "").trim().toLocaleLowerCase("tr-TR").replace(/\s+/g, " ");
}

function salaryPeriodKey(person, period) {
  const normalized = dateOnly(period);
  return (person?.pay_type || "monthly") === "weekly" ? normalized : monthOf(normalized);
}

function readSalaryPaidMarkers() {
  try {
    const raw = JSON.parse(localStorage.getItem(SALARY_PAID_MARKERS_KEY) || "[]");
    if (!Array.isArray(raw)) return [];
    const cutoff = Date.now() - SALARY_PAID_MARKER_MAX_AGE_MS;
    return raw.filter((row) => Number(row?.committed_at || 0) >= cutoff && row?.period_key);
  } catch (_) {
    return [];
  }
}

function writeSalaryPaidMarkers(rows) {
  try {
    localStorage.setItem(SALARY_PAID_MARKERS_KEY, JSON.stringify((rows || []).slice(-400)));
  } catch (_) {}
}

function rememberPersistentSalaryPaid(person, period, payment = null) {
  const periodKey = salaryPeriodKey(person, period);
  if (!periodKey) return;
  const personId = String(person?.id ?? payment?.person_id ?? "");
  const personName = normalizePayrollName(person?.name || payment?.person_name || "");
  const payType = (person?.pay_type || "monthly") === "weekly" ? "weekly" : "monthly";
  let rows = readSalaryPaidMarkers();
  rows = rows.filter((row) => {
    if (String(row.period_key) !== String(periodKey) || String(row.pay_type || "monthly") !== payType) return true;
    const sameId = personId && String(row.person_id || "") === personId;
    const sameName = personName && normalizePayrollName(row.person_name) === personName;
    return !(sameId || sameName);
  });
  rows.push({
    person_id: personId,
    person_name: person?.name || payment?.person_name || "",
    pay_type: payType,
    period_key: periodKey,
    pay_period: dateOnly(period),
    committed_at: Date.now()
  });
  writeSalaryPaidMarkers(rows);
}

function isPersistentlySalaryPaid(person, period) {
  const periodKey = salaryPeriodKey(person, period);
  if (!periodKey) return false;
  const personId = String(person?.id ?? "");
  const personName = normalizePayrollName(person?.name || "");
  const payType = (person?.pay_type || "monthly") === "weekly" ? "weekly" : "monthly";
  return readSalaryPaidMarkers().some((row) => {
    ...
    return Boolean(personName && normalizePayrollName(row.person_name) === personName);
  });
}

function salaryCommitKey(personId, period, payType = "monthly") {
  const normalizedPeriod = dateOnly(period);
  return `${String(personId)}|${payType === "weekly" ? normalizedPeriod : monthOf(normalizedPeriod)}`;
}

function rememberSalaryCommit(payment, person) {
  const normalized = {
    ...payment,
    person_id: payment?.person_id ?? person?.id ?? "",
    pay_period: dateOnly(payment?.pay_period) || todayISO,
    _clientCommittedAt: Date.now()
  };
  const key = salaryCommitKey(normalized.person_id, normalized.pay_period, person?.pay_type || "monthly");
  recentSalaryCommits.set(key, normalized);
  rememberPersistentSalaryPaid(person, normalized.pay_period, normalized);
  return normalized;
}

function mergeSalaryCommits(serverPayments = [], people = state.people) {
  const now = Date.now();
  const merged = [...serverPayments];

  for (const [key, localPayment] of [...recentSalaryCommits.entries()]) {
    if (now - Number(localPayment._clientCommittedAt || 0) > SALARY_COMMIT_GUARD_MS) {
      recentSalaryCommits.delete(key);
      continue;
    }

    const person = people.find((row) => String(row.id) === String(localPayment.person_id));
    const weekly = (person?.pay_type || "monthly") === "weekly";
    const exists = merged.some((row) => {
      if (String(row.person_id) !== String(localPayment.person_id)) return false;
      return weekly
        ? dateOnly(row.pay_period) === dateOnly(localPayment.pay_period)
        : monthOf(row.pay_period) === monthOf(localPayment.pay_period);
    });
...
function paymentFor(personId, period) {
  const person = getPerson(personId);
  if (person && isPersistentlySalaryPaid(person, period)) {
    return { person_id: person.id, pay_period: dateOnly(period), _persistent_paid_marker: true };
  }
  const weekly = (person?.pay_type || "monthly") === "weekly";
...
async function loadAll({ quiet = false, preservePerson = true } = {}) {
  ...
  try {
    ...
    const serverSalaryPayments = (payload.salary_payments || []).map((payment) => ({
      ...payment,
      pay_period: dateOnly(payment.pay_period)
    }));
    state.salaryPayments = mergeSalaryCommits(serverSalaryPayments, state.people);
    ...
    state.salaryTableReady = false;
    $("migrationWarning")?.classList.remove("hidden");
    showToast(`Avans & Maaş verileri alınamadı: ${error.message || "API hatası"}`);
  }
}

function fillPersonSelect() {
  const select = $("activePerson");
  const activePeople = state.people.filter((person) => person.is_active !== false);
  select.innerHTML = activePeople.length ? activePeople.map((person) => `<option value="${person.id}" ${String(person.id) === String(state.selectedPersonId) ? "selected" : ""}>${person.name}</option>`).join("") : '<option value="">Önce personel kaydet</option>';
  if (!activePeople.some((person) => String(person.id) === String(state.selectedPersonId))) {
    state.selectedPersonId = activePeople[0]?.id || "";
  }
...
function renderPeople() {
  const activePeople = state.people.filter((person) => person.is_active !== false);
  $("personList").innerHTML = activePeople.length ? activePeople.map((person) => `<div class="person-item"><div><strong>${person.name}</strong><span>${money.format(person.salary || 0)} · ${payDayText(person)}</span></div><div class="person-actions"><button type="button" class="ghost" data-edit-person="${person.id}">Düzenle</button><button type="button" class="danger" data-delete-person="${person.id}">Sil</button></div></div>`).join("") : '<p class="empty">Henüz personel kaydı yok.</p>';
  ...
  document.querySelectorAll("[data-delete-person]").forEach((button) => button.addEventListener("click", () => deletePerson(button.dataset.deletePerson)));
}
...
async function saveAdvance() {
  ...
  if (payrollActionLock.advance) return;

  const button = $("saveAdvanceBtn");
  payrollActionLock.advance = true;
  if (button) { button.disabled = true; button.textContent = "Kaydediliyor…"; }

  try {
    await apiFetch("/api/kasaflow/payroll/advances", {
      method: "POST",
      body: { person_id: personId, amount, note, advance_date: advanceDate }
    });
    $("advanceAmount").value = "";
    $("advanceNote").value = "";
    await loadAll({ quiet: true });
    showToast("Avans kaydedildi; kesilecek tutara eklendi.");
  } catch (error) {
    showToast(`Avans kaydedilemedi: ${error.message || "API hatası"}`);
  } finally {
    payrollActionLock.advance = false;
    if (button) { button.disabled = false; button.textContent = "Avansı Kaydet"; }
  }
}
...
Maaş yatırıldı olarak işaretlensin mi?`)) return;
  if (payrollActionLock.salary) return;

  const button = $("markSalaryPaidBtn");
  payrollActionLock.salary = true;
  if (button) { button.disabled = true; button.textContent = "Kaydediliyor…"; }

  try {
    const saveResult = await apiFetch("/api/kasaflow/payroll/salary-payments", {
      method: "PUT",
      body: {
        person_id: person.id,
        pay_period: period,
        deductions: rows.map((row) => ({ advance_id: row.advanceId, amount: row.amount }))
      }
    });

    // Backend farklı bir response şekli döndürse bile HTTP/API başarıdan sonra
    // ödeme artık tamamlanmış kabul edilir. Ekranı response.payment alanına bağımlı bırakmıyoruz.
    const apiPayment =
      saveResult?.payment ||
      saveResult?.salary_payment ||
      saveResult?.data?.payment ||
      saveResult?.data?.salary_payment ||
      saveResult?.row ||
      null;

    // İstek yapılan dönem esas alınır; backend response içindeki tarih farklı formatta olsa bile
    // kullanıcıya ait gerçek bekleyen dönem kapanır.
    const committedPayment = rememberSalaryCommit({
      ...(apiPayment || {}),
      person_id: apiPayment?.person_id ?? person.id,
      pay_period: period,
      gross_salary: apiPayment?.gross_salary ?? Number(person.salary || 0),
      advance_deduction: apiPayment?.advance_deduction ?? deductionTotal,
      net_paid: apiPayment?.net_paid ?? netPaid,
      paid_at: apiPayment?.paid_at || apiPayment?.created_at || new Date().toISOString(),
      created_at: apiPayment?.created_at || new Date().toISOString()
    }, person);

    // Aynı dönem için eski satırı kaldır, kesinleşen satırı ekle ve hemen yeniden çiz.
    state.salaryPayments = state.salaryPayments.filter((row) => {
      if (String(row.person_id) !== String(person.id)) return true;
      return (person.pay_type || "monthly") === "weekly"
        ? dateOnly(row.pay_period) !== dateOnly(period)
        : monthOf(row.pay_period) !== monthOf(period);
    });
    state.salaryPayments.push(committedPayment);
    renderAll();
    ...
    // Server ile tekrar eşitle ama commit koruması stale GET'in ödemeyi geri açmasını engeller.
    setTimeout(() => {
      loadAll({ quiet: true }).catch(() => {});
    }, 1200);
  } catch (error) {
    showToast(`Maaş kaydedilemedi: ${error.message || "API hatası"}`);
  } finally {
    payrollActionLock.salary = false;
    if (button) { button.disabled = false; button.textContent = "✓ Maaşı Yattı"; }
  }
}

function resetPersonForm() {
  state.editingPersonId = ""; $("personName").value = ""; $("personSalary").value = ""; $("personPayType").value = "monthly"; $("personSalaryDay").value = "15"; $("personSalaryWeekday").value = "1"; $("personTrackingStart").value = todayISO; $("cancelPersonEditBtn").classList.add("hidden"); $("savePersonBtn").textContent = "Personeli Kaydet"; togglePayFields();
}

function editPerson(id) {
  const person = getPerson(id); if (!person) return;
  state.editingPersonId = person.id; $("personName").value = person.name || ""; $("personSalary").value = person.salary || 0; $("personPayType").value = person.pay_type || "monthly"; $("personSalaryDay").value = String(person.salary_day || 1); $("personSalaryWeekday").value = String(person.salary_weekday ?? 1); $("personTrackingStart").value = dateOnly(person.salary_tracking_start) || todayISO; $("cancelPersonEditBtn").classList.remove("hidden"); $("savePersonBtn").textContent = "Değişiklikleri Kaydet"; togglePayFields(); window.scrollTo({ top: 0, behavior: "smooth" });
}
...
async function savePerson() {
  ...
  const payload = { name, salary, pay_type: payType, salary_day: payType === "monthly" ? Number($("personSalaryDay").value || 1) : null, salary_weekday: payType === "weekly" ? Number($("personSalaryWeekday").value ?? 1) : null, salary_tracking_start: $("personTrackingStart").value || todayISO, is_active: true };

  try {
    if (state.editingPersonId) {
      await apiFetch(`/api/kasaflow/payroll/people/${encodeURIComponent(state.editingPersonId)}`, { method: "PATCH", body: payload });
    } else {
      await apiFetch("/api/kasaflow/payroll/people", { method: "POST", body: payload });
    }
    resetPersonForm();
    await loadAll({ quiet: true });
    showToast("Personel kaydedildi.");
  } catch (error) {
    showToast(`Personel kaydedilemedi: ${error.message || "API hatası"}`);
  }
}
...
async function deletePerson(id) {
  ...
  const openAdvance = advancesFor(person.id).reduce((sum, advance) => sum + remainingOf(advance), 0);
  const extra = openAdvance > 0 ? `\n\nBu personelin ${money.format(openAdvance)} açık avansı var. Geçmiş kayıtlar korunacak.` : "\n\nGeçmiş maaş ve avans kayıtları korunacak.";
  const ok = window.confirm(`${person.name} personel listesinden silinsin mi?${extra}`);
  if (!ok) return;

  try {
    // Muhasebe geçmişini korumak için fiziksel kayıt silinmez; aktif personel
    // listesinden arşivlenir ve uygulamada artık gösterilmez.
    await apiFetch(`/api/kasaflow/payroll/people/${encodeURIComponent(id)}`, {
      method: "PATCH",
      body: { is_active: false }
    });
    if (String(state.editingPersonId) === String(id)) resetPersonForm();
    if (String(state.selectedPersonId) === String(id)) state.selectedPersonId = "";
    await loadAll({ quiet: true, preservePerson: false });
    showToast(`${person.name} personel listesinden silindi.`);
  } catch (error) {
    showToast(`Personel silinemedi: ${error.message || "API hatası"}`);
  }
}
