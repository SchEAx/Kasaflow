console.info("[KasaFlow Payroll] PostgreSQL API build 2.3.10");
...
Maaş yatırıldı olarak işaretlensin mi?`)) return;
  ...
  try {
    ...
    // API başarı döndürdüğü anda ödemeyi lokal state'e işle.
    // Böylece CDN/proxy tarafında milisaniyelik eski GET cevabı gelse bile
    // kullanıcı ödediği maaşı listede tekrar görmez.
    if (saveResult?.payment) {
      const saved = {
        ...saveResult.payment,
        pay_period: dateOnly(saveResult.payment.pay_period) || period
      };
      state.salaryPayments = state.salaryPayments.filter((row) => !(
        String(row.person_id) === String(person.id) &&
        dateOnly(row.pay_period) === dateOnly(period)
      ));
      state.salaryPayments.push(saved);
      renderAll();
    }
    ...
    // Commit sonrası yetkili kaynaktan tekrar doğrula; GET cache-buster içerir.
    setTimeout(() => {
