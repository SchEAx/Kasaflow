console.info("[KasaFlow Payroll] PostgreSQL API build 2.3.12");
...
async function togglePerson(id) {
  ...
  try {
    await apiFetch(`/api/kasaflow/payroll/people/${encodeURIComponent(id)}`, {
      method: "PATCH",
      body: { is_active: person.is_active === false }
    });
    await loadAll({ quiet: true });
    showToast("Personel durumu güncellendi.");
  } catch (error) {
    showToast(`Personel durumu değiştirilemedi: ${error.message || "API hatası"}`);
  }
}
